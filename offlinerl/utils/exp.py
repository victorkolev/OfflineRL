...
def set_free_device_fn():
    device = (
        "cuda" + ":" + str(select_free_cuda()) if torch.cuda.is_available() else "cpu"
    )
...
def init_tb_logger(repo=None, experiment_name=None):
     if repo is None:
          repo = log_path()
     tb_logger = SummaryWriter(os.path.join(repo, experiment_name))
     return tb_logger
