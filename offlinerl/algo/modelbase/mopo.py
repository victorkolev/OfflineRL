...
def algo_init(args):
    ...
    transition_optim = torch.optim.AdamW(
        transition.parameters(), lr=args["transition_lr"], weight_decay=0.000075
    )
...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        self.args["buffer_size"] = (
            int(self.args["data_collection_per_epoch"]) * self.args["horizon"] * 5
        )
    ...
    def train(self, train_buffer, val_buffer, callback_fn):
        if self.args["dynamics_path"] is not None:
            ...
            if self.args["dynamics_save_path"] is None:
                self.args["dynamics_save_path"] = os.path.join(
                    self.models_save_dir, "dynamics.pt"
                )
            ...
            with torch.no_grad():
                ...
                for t in range(self.args["horizon"]):
                    ...
                    aleatoric_uncertainty = torch.max(
                        torch.norm(next_obs_dists.stddev, dim=-1, keepdim=True), dim=0
                    )[0]
                    uncertainty = (
                    ...
                    penalized_reward = (
                        reward - self.args["lam"] * uncertainty - self.args["dumbbell"]
                    )
    ...
    def _sac_update(self, batch_data):
        ...
        # soft target update
        self._sync_weight(
            self.target_q1, self.q1, soft_target_tau=self.args["soft_target_tau"]
        )
        self._sync_weight(
            self.target_q2, self.q2, soft_target_tau=self.args["soft_target_tau"]
        )
        ...
        actor_loss = (
            -q.mean() + torch.exp(self.log_alpha) * action_log_prob.sum(dim=-1).mean()
        )
    ...
    def _eval_transition(self, transition, valdata):
        with torch.no_grad():
            ...
            loss = (
                (dist.mean - torch.cat([valdata["obs_next"], valdata["rew"]], dim=-1))
                ** 2
            ).mean(dim=(1, 2))
