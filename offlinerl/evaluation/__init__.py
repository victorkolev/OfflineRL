...
class OnlineCallBackFunction(CallBackFunction):
    def initialize(
        self, train_buffer, val_buffer, task, number_of_runs=100, *args, **kwargs
    ):
        self.task = task
    ...
    def __call__(self, policy) -> dict:
        ...
        eval_res = OrderedDict()
        if "d4rl" in self.args.get("task", ""):
            eval_fn = d4rl_eval_fn(self.args["task"])
            eval_res.update(eval_fn(policy))
        else:
            if not ray.is_initialized():
                ray.init(ignore_reinit_error=True)
            eval_res.update(
                test_on_real_env(policy, self.env, number_of_runs=self.number_of_runs)
            )
        return eval_res
...
class FQECallBackFunction(CallBackFunction):
    def initialize(
        ...
        **kwargs,
        ...
        assert (
            train_buffer is not None or val_buffer is not None
        ), "you need to provide at least one buffer to run FQE test"
    ...
    def __call__(self, policy) -> dict:
        ...
        if self.start_index is not None:
            ...
            batches = [
                torch.tensor(self.buffer.sample(256).obs).float() for _ in range(100)
            ]
...
class MBOPECallBackFunction(CallBackFunction):
    def initialize(self, train_buffer=None, val_buffer=None, *args, **kwargs):
        assert (
            train_buffer is not None or val_buffer is not None
        ), "you need to provide at least one buffer to run MBOPE test"
        self.buffer = val_buffer or train_buffer
        ...
        data = self.buffer[0]
        self.trainsition = MLP(
            data.obs.shape[-1] + data.act.shape[-1], data.obs.shape[-1] + 1, 512, 3
        ).to(device)
...
class AutoOPECallBackFunction(CallBackFunction):
    def initialize(self, train_buffer=None, val_buffer=None, *args, **kwargs):
        assert (
            train_buffer is not None or val_buffer is not None
        ), "you need to provide at least one buffer to run AutoOPE test"
        self.buffer = val_buffer or train_buffer
        ...
        class AutoRegressiveTransition(torch.nn.Module):
            def __init__(
                ...
                if with_reward:
                    self.one_hot = torch.cat(
                        [self.one_hot, torch.ones((1, obs_dim), device=device)], dim=0
                    )
            ...
            def forward(self, s, a):
                ...
                for i in range(self.obs_dim):
                    one_hot = F.one_hot(
                        torch.tensor(i, device=self.device), num_classes=self.obs_dim
                    )
            ...
            def forward_r(self, s, a):
                ...
                final_all_one = torch.ones(
                    (sa.shape[0], self.obs_dim), device=self.device
                )
            ...
            def log_prob_without_r(self, s, a, s_):
                sa = torch.cat([s, a], dim=-1)
                maked_s_ = (
                    torch.unsqueeze(s_, dim=1).expand(-1, self.obs_dim, -1) * self.mask
                )
                ...
                # 沿着batch维复制
                one_hot = torch.unsqueeze(self.one_hot, dim=0).expand(
                    sa.shape[0], -1, -1
                )
            ...
            def log_prob_r(self, s, a, s_, r):
                ...
                maked_s_ = (
                    torch.unsqueeze(s_, dim=1).expand(-1, self.obs_dim + 1, -1)
                    * self.mask
                )
            ...
            def input_check(self, *input_data):
                ...
                for each_data in input_data:
                    if isinstance(each_data, np.ndarray):
                        each_data = torch.from_numpy(each_data.astype(np.float32)).to(
                            self.device
                        )
                    if each_data.dim() == 1:
        ...
        optim_Auto = torch.optim.Adam(
            self.trainsition.parameters(), lr=1e-3, weight_decay=float(1e-6)
        )
        scheduler_Auto = torch.optim.lr_scheduler.StepLR(
            optim_Auto, step_size=5, gamma=0.99
        )
        ...
        for i in tqdm(range(100_0000)):
            ...
            nll_Auto = -torch.mean(
                self.trainsition.log_prob_r(
                    data_train.obs, data_train.act, data_train.obs_next, data_train.rew
                )
            )
            ...
            if i % 1000 == 0:
                ...
                nll_val_Auto = -torch.mean(
                    self.trainsition.log_prob_r(
                        data_val.obs, data_val.act, data_val.obs_next, data_val.rew
                    )
                )
                print(
                    "batch_num : {} | lr : {:.2e} |\tAuto : nll_val {:.2f} |".format(
    ...
    def __call__(self, policy) -> dict:
        ...
        res = OrderedDict()
        res["Auto-OPE"] = (
            (torch.nansum(ret) / (1 - torch.isnan(ret).float()).sum()).cpu().item()
        )
        return res
